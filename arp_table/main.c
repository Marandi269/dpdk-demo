...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(172, 16, 77, 130);
...
static int ng_encode_icmp_pkt(uint8_t *msg, uint8_t *dst_mac,
                              uint32_t sip, uint32_t dip, uint16_t id, uint16_t seqnb, const unsigned char* padding) {
    ...
    memset((void*)&icmp[1], 'A', 56);
    memccpy((void*)&icmp[1], padding, 1, 56);
    icmp->icmp_cksum = 0;
    icmp->icmp_cksum = ng_checksum((uint16_t*)icmp, sizeof(struct rte_icmp_hdr) + 56);
...
static struct rte_mbuf *ng_send_icmp(struct rte_mempool *mbuf_pool, uint8_t *dst_mac,
    ...
    uint8_t *pkt_data = rte_pktmbuf_mtod(mbuf, uint8_t *);
    ng_encode_icmp_pkt(pkt_data, dst_mac, sip, dip, id, seqnb, padding);
...
arp_request_timer_cb(__attribute__((unused)) struct rte_timer *tim,
                     void *arg) {
                        printf("arp_request_timer_cb arg: %p\n", arg);
                        return;
        ...
        for (i = 0;i < num_recvd;i ++) {
            ...
            if (iphdr->next_proto_id == IPPROTO_ICMP) {
                ...
                if (icmphdr->icmp_type == RTE_IP_ICMP_ECHO_REQUEST) {
                    ...
                    unsigned char *p = (void *)&icmphdr[1];
                    for(int i=0; i< 56; i++){
                        printf("%x\t", (int)p[i]);
                        if((i-1)%16==0){
                            printf("\n");
                    ...
                    struct rte_mbuf *txbuf = ng_send_icmp(mbuf_pool, ehdr->s_addr.addr_bytes,
                                                          iphdr->dst_addr, iphdr->src_addr, icmphdr->icmp_ident, icmphdr->icmp_seq_nb, p);
