...
function<void(rte_ether_hdr *ehdr, struct rte_mbuf *pBuf, Dpdk dpdk)>
    ether_empty_process = [](rte_ether_hdr *ehdr, struct rte_mbuf *pBuf,
                             Dpdk dpdk) { cout << "empty handler..." << endl; };

map<uint32_t, ArpRecord> Dpdk::arpTable = {};

int encode_arp_pkt(uint8_t *msg, uint16_t opcode, uint8_t *dst_mac,
                   uint32_t sip, uint32_t dip, Dpdk dpdk) {
  // 1 ethhdr
  struct rte_ether_hdr *eth = (struct rte_ether_hdr *)msg;
  rte_memcpy(eth->s_addr.addr_bytes, dpdk.gSrcMac, RTE_ETHER_ADDR_LEN);
  if (!strncmp((const char *)dst_mac, (const char *)dpdk.gDefaultArpMac,
               RTE_ETHER_ADDR_LEN)) {
  ...
  rte_memcpy(arp->arp_data.arp_sha.addr_bytes, dpdk.gSrcMac,
             RTE_ETHER_ADDR_LEN);
...
struct rte_mbuf *send_arp(struct rte_mempool *mbuf_pool, uint16_t opcode,
...
void arp_process(rte_ether_hdr *ehdr, struct rte_mbuf *pBuf, Dpdk dpdk) {
  ...
  dpdk.arpTable[ahdr->arp_data.arp_sip] =
  ...
  if (ahdr->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REQUEST)) {
    ...
    struct rte_mbuf *arpbuf = send_arp(
        dpdk.mbuf_pool, RTE_ARP_OP_REPLY, ahdr->arp_data.arp_sha.addr_bytes,
...
static int encode_icmp_pkt(uint8_t *msg, uint8_t *dst_mac, uint32_t sip,
                           uint32_t dip, uint16_t id, uint16_t seqnb,
                           const unsigned char *padding, Dpdk dpdk) {
  ...
  ip->total_length =
      htons(sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_icmp_hdr) + 56);
  ip->packet_id = 0;
  ...
  struct rte_icmp_hdr *icmp =
      (struct rte_icmp_hdr *)(msg + sizeof(struct rte_ether_hdr) +
  ...
  icmp->icmp_cksum = 0;
  icmp->icmp_cksum =
      icmp_checksum((uint16_t *)icmp, sizeof(struct rte_icmp_hdr) + 56);

  return 0;
}

static struct rte_mbuf *send_icmp(struct rte_mempool *mbuf_pool,
                                  uint8_t *dst_mac, uint32_t sip, uint32_t dip,
...
static int encode_udp_pkt(uint8_t *msg, unsigned char *data, uint16_t total_len,
                          Dpdk dpdk) {
  ...
  struct rte_udp_hdr *udp =
      (struct rte_udp_hdr *)(msg + sizeof(struct rte_ether_hdr) +
                             sizeof(struct rte_ipv4_hdr));
  udp->src_port = dpdk.port;
  udp->dst_port = dpdk.dstPort;
  uint16_t udplen =
      total_len - sizeof(struct rte_ether_hdr) - sizeof(struct rte_ipv4_hdr);
...
void ipv4_process(rte_ether_hdr *ehdr, struct rte_mbuf *pBuf, Dpdk dpdk) {
  struct rte_ipv4_hdr *iphdr = rte_pktmbuf_mtod_offset(
      pBuf, struct rte_ipv4_hdr *, sizeof(struct rte_ether_hdr));
  if (iphdr->next_proto_id == IPPROTO_ICMP) {
    ...
    if (icmphdr->icmp_type == RTE_IP_ICMP_ECHO_REQUEST) {
      ...
      struct rte_mbuf *txbuf =
          send_icmp(dpdk.mbuf_pool, ehdr->s_addr.addr_bytes, iphdr->dst_addr,
...
void Dpdk::port_init() {
  ...
  if (rte_eth_rx_queue_setup(this->portId, 0, 1024,
                             rte_eth_dev_socket_id(this->portId), NULL,
                             mbuf_pool) < 0) {
...
void Dpdk::processSomePkts() {
  ...
  for (int i = 0; i < num_recvd; i++) {
    ...
    function<void(rte_ether_hdr * ehdr, struct rte_mbuf * pBuf, Dpdk dpdk)>
        handler = etherProcessFuncs[(ehdr->ether_type)];
